...
from pybullet_tools.utils import child_link_from_joint, draw_pose, get_movable_joints, set_base_values, joint_from_name, quat_from_euler, set_configuration, set_joint_position, \
...
def test_drake_base_motion(pr2, base_start, base_goal, obstacles=[], door=None):
    ...
    print('!!!!!!!!!!!!!!!!', base_joints, base_goal)
    base_goal = base_goal[:len(base_joints)]
    print('!!!!!!!!!!!!!!!!', base_joints, base_goal)
    # # 不懂，从basejoints到basegoal?
    
    wait_if_gui('Plan Base?')
    with LockRenderer(lock=False):
        base_path = plan_joint_motion(pr2, base_joints, base_goal, obstacles=obstacles,custom_limits = custom_limits,
                                      disabled_collisions=disabled_collisions,restarts=4, max_iterations=50, smooth=50)
...
def main(use_pr2_drake=True):
    ...
    door = create_door(length=2, height=2)
    set_point(door, [0, 0, 0])
    ...
    test_drake_base_motion(pr2, base_start, base_goal, obstacles=obstacles, door = door)
