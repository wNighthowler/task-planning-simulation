...
class Trajectory(Command):
    ...
    def apply(self, state, sample=1):
        # demo的command最终调用的apply是这个
        # print('trajectory.apply')
        handles = add_segments(self.to_points()) if self._draw and has_gui() else []
        # 下面的for循环是仿真重点
        # 以sample为步长，从self.path的list中取conf来仿真；conf是pr2_primitives.Conf
        # conf.assgin()应该就是重点，每一步都assign当前的conf来指导机器人移动
        # conf就是当前机器人的conf
...
def get_ir_sampler(problem, custom_limits={}, max_attempts=25, collisions=True, learned=True):
    ...
    def gen_fn(arm, obj, pose, grasp):
        ...
        print(custom_limits, lower_limits, upper_limits)
        while True:
            count = 0
...
def get_ik_fn(problem, custom_limits={}, collisions=True, teleport=False):
    ...
    def fn(arm, obj, pose, grasp, base_conf):
        ...
        # conf = Conf(robot, arm_joints, approach_conf2)
        return (cmd,)
    return fn
...
def get_motion_gen(problem, custom_limits={}, collisions=True, teleport=False, restarts = 4, max_iterations = 50, smooth = 50, algorithm = None):
    ...
    def fn(bq1, bq2, fluents=[]):
        ...
        if teleport:
            ...
            raw_path = plan_joint_motion(robot, bq2.joints, bq2.values, attachments=[],
                                         ...
                                         restarts=restarts, max_iterations=max_iterations, smooth=smooth, algorithm=algorithm)
