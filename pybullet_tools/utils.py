...
def set_joint_position(body, joint, value):
    ...
    # p.resetJointState就是设置机器人的配置，让机器人处于某个配置，具体仿真的时候就是设置一大堆的这个值
...
# def merge_bodies(body_ids):
#     """
#     合并多个物体为一个单一的多体对象，考虑每个物体已设置的全局位置。
#     参数:
#         body_ids: 要合并的物体 ID 列表
#     返回:
#         新创建的单一多体对象 ID
#     """
#     links = []
#     base_body_id = body_ids[0]  # 选择第一个物体作为基物体

#     # 获取 base_link 属性
#     base_mass, base_collision_id, base_visual_id, base_point, base_quat = get_body_properties(base_body_id)

#     # 遍历其他物体作为 links
#     for body_id in body_ids[1:]:
#         mass, collision_id, visual_id, child_point, child_quat = get_body_properties(body_id)

#         # 计算相对于 base_link 的位置和朝向
#         relative_point, relative_quat = calculate_relative_transform(base_point, base_quat, child_point, child_quat)
...
#             quat=relative_quat,
#             parent=0,  # 所有链接直接连接到 base
#             joint_type=p.JOINT_FIXED,  # 固定连接
#             joint_axis=[0, 0, 0]
#         ))

#     # 创建新的多体
#     new_body_id = create_multi_body(base_link=LinkInfo(
#         mass=base_mass,
#         collision_id=base_collision_id,
#         visual_id=base_visual_id,
#         point=base_point,
#         quat=base_quat
#     ), links=links)
...
# def get_body_properties(body_id):
#     """
#     获取物体的属性，包括质量、碰撞形状、可视形状、位置和朝向。
#     """
#     mass = p.getDynamicsInfo(body_id, -1)[0]  # 获取质量
#     collision_id = p.getCollisionShapeData(body_id, -1)[0][2]  # 碰撞形状
#     visual_id = p.getVisualShapeData(body_id)[0][2]  # 可视形状
#     pos, orn = p.getBasePositionAndOrientation(body_id)  # 位置和朝向
#     return mass, collision_id, visual_id, pos, orn


# def calculate_relative_transform(base_pos, base_quat, child_pos, child_quat):
#     """
#     计算相对于基坐标的位姿。
#     """
#     inv_base_pos, inv_base_quat = p.invertTransform(base_pos, base_quat)
#     relative_pos, relative_quat = p.multiplyTransforms(inv_base_pos, inv_base_quat, child_pos, child_quat)
#     return relative_pos, relative_quat


# # 定义 LinkInfo 数据结构
# class LinkInfo:
#     def __init__(self, mass=0, collision_id=None, visual_id=None, point=[0, 0, 0], quat=[0, 0, 0, 1],
#                  parent=None, joint_type=p.JOINT_FIXED, joint_axis=[0, 0, 0], inertial_point=[0, 0, 0],
#                  inertial_quat=[0, 0, 0, 1]):
#         self.mass = mass
...
#         self.inertial_point = inertial_point
#         self.inertial_quat = inertial_quat
...
def aabb_overlap(aabb1, aabb2):
    ...
    # time_end = time.time()
    # print('aabb_overlap: ', time_end - time_start)
...
def any_link_pair_collision(body1, links1, body2, links2=None, **kwargs):
    ...
    # 0.010, 因为第一轮的时候执行了超级多次的pairwise_link_collision
    # 因为第一遍的时候，links2长度很长，有81个，而后续的阶段links2只有1个，所以全排列下数量差别巨大
...
def pairwise_collision(body1, body2, **kwargs):
    if isinstance(body1, tuple) or isinstance(body2, tuple):
        ...
        # 第一遍0.01s
        result = any_link_pair_collision(body1, links1, body2, links2, **kwargs)
        return result
...
def get_collision_fn(body, joints, obstacles=[], attachments=[], self_collisions=True, disabled_collisions=set(),
    ...
    def collision_fn(q, verbose=False):
        ...
        if limits_fn(q):
            return True
        # collision每一次需要设置机器人配置来仿真，检测碰撞检测，但是RRT具体耗时是在其中的一个细节
        set_joint_positions(body, joints, q)
        for attachment in attachments:
            attachment.assign()
        # wait_for_duration(1e-7)
        ...
        # 这个部分的仿真非常慢，来到了0.1s
        # print('moving: ', moving_bodies)
        # print('obstacles: ', obstacles)
        # a = input()
        for body1, body2 in product(moving_bodies, obstacles):
            # 第一次的循环花了0.010s，而后面大约30次的循环每一次只有0.0001s，第一次循环导致了很长的时间
            # 就是第一次pairwise_collision会消耗0.01s左右，后续变成0.001s左右
